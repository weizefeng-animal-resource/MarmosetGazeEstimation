...
def annotate_video(calibrated_result_file_name, marmoset_video_name, stimuli_video_name, output_file_name, frame_delay, accuracy_cutoff):
    try:
        assert frame_delay >= 0
    except AssertionError:
        print('Frame delay is invalid.')
        return
    try:
        data_frame = pd.read_csv('configs.csv', header=0)
        crop_x = float(data_frame['crop start x (pixel)'][0])
        crop_y = float(data_frame['crop start y (pixel)'][0])
    except Exception as e:
        print('Configs.csv is not found or is invalid:', e)
        return
    print('Processing ' + calibrated_result_file_name + ' ... ', end='')
    try:
        data_frame = pd.read_csv(os.path.join('DeepLabCutOutputs', calibrated_result_file_name), header=1)
        facial_landmark_coords = np.float32(data_frame[['Amount', 'Amount.1', 'Nose', 'Nose.1', 'Right corner of eye', 'Right corner of eye.1', 'Left corner of eye', 'Left corner of eye.1', 'Mouth', 'Mouth.1', 'Right corner of mouth', 'Right corner of mouth.1', 'Left corner of mouth', 'Left corner of mouth.1']][1:])
        facial_landmark_coords[:, 0::2] += crop_x
        facial_landmark_coords[:, 1::2] += crop_y
        facial_landmark_coords = np.int64(facial_landmark_coords)
        is_accurate = np.float32(data_frame[['Amount.2', 'Nose.2', 'Right corner of eye.2', 'Left corner of eye.2', 'Mouth.2', 'Right corner of mouth.2', 'Left corner of mouth.2']][1:])
        is_accurate = is_accurate > accuracy_cutoff
    except Exception as e:
        print('failed, error message: ' + str(e))
        return
    try:
        data_frame = pd.read_csv(os.path.join('CalibratedResults', calibrated_result_file_name), header=0)
    ...
    if not marmoset_video.isOpened():
        print('failed.')
        return
    video_width = int(marmoset_video.get(cv2.CAP_PROP_FRAME_WIDTH))
    video_height = int(marmoset_video.get(cv2.CAP_PROP_FRAME_HEIGHT))
    video_fps = float(marmoset_video.get(cv2.CAP_PROP_FPS))
    ...
    try:
        ...
        print('Marmoset video and stimuli video should have the same number of frames.')
        return
    if 'camera1.pickle' not in os.listdir():
        print('camera1.pickle is not found.')
        return
    with open('camera1.pickle', 'rb') as f:
        _, camera_matrix, distortion_coefficients = pickle.load(f)
    save_file_path = os.path.join('AnnotatedVideos', output_file_name)
    fourcc = cv2.VideoWriter_fourcc('m', 'p', '4', 'v')
    writer = cv2.VideoWriter(save_file_path, fourcc, video_fps, (video_width, video_height))
    ...
    for i in range(frame_delay, total_frame_number):
        ...
        if np.sum(is_accurate[i]) > 5:
            ...
            if not np.isnan(gaze_points[i]).any():
                cv2.circle(frame_stimuli, np.int64(gaze_points[i]), 15, (0, 0, 255), -1)
        frame_stimuli = cv2.resize(frame_stimuli, (480, 270))
        frame_marmoset[-270:, -480:] = frame_stimuli
        writer.write(frame_marmoset)
    marmoset_video.release()
    stimuli_video.release()
    writer.release()
    print('Annotated video is generated successfully.')
...
def calibrate_camera(dir_name, pattern_size=8, calibrate_distortion=False):
    ...
    if len(img_points) == 0:
        print('No chessboard is detected.')
        return None
...
class CalibrateEstimator:
    ...
    def calibrate_gaze_points(self, gaze_points, affine_transform=None):
        if affine_transform is None:
            if self.affine_transform is None:
                print('Estimator is not calibrated.')
                return
    ...
    def estimate_affine_matrix(self, actual_points, gaze_points):
        ...
        result = model.fit(actual_points=actual_points, gaze_points=gaze_points, data=np.zeros(gaze_points.shape), params=parameters)
        self.affine_transform = np.float32([[result.values['a11'], result.values['a12'], result.values['tx']], [result.values['a21'], result.values['a22'], result.values['ty']], [0, 0, 1]])
...
def calibrate_estimator():
    ...
    try:
        ...
        frame_number_list = np.int64(data_frame['frame number'])
    except Exception as e:
        print('Calibration_configs.csv is not found or is invalid:', e)
        return
    gaze_points = []
    flags = []
    ...
    if len(gaze_points) == 0:
        print('No gaze point is available.')
        return
...
def calibrate_results():
    try:
        data_frame = pd.read_csv('configs.csv', header=0)
        video_width = float(data_frame['stimuli video width (pixel)'][0])
        video_height = float(data_frame['stimuli video height (pixel)'][0])
    except Exception:
        print('Configs.csv is not found or is invalid.')
        return
    if 'calibrator.pickle' not in os.listdir():
        print('Calibrator.pickle is not found.')
        return
...
def create_directories():
    ...
    data_frame = pd.DataFrame([[1920, 1080, 460, 20]], columns=['stimuli video width (pixel)', 'stimuli video height (pixel)', 'crop start x (pixel)', 'crop start y (pixel)'])
    data_frame.to_csv('configs.csv', index=False)
    data_frame = pd.DataFrame(columns=['result csv file', 'frame number', 'video width (pixel)', 'video height (pixel)', 'solid white circle x (pixel)', 'solid white circle y (pixel)'])
    data_frame.to_csv('calibration_configs.csv', index=False)
    face_model = np.float32([[0, -0.436, -1.165], [0, 0.68, -1.54], [-1.325, 0, 0], [1.325, 0, 0], [0, 1.41, -1.613], [-1.058, 1.695, -0.381], [1.058, 1.695, -0.381]]).T
    data_frame = pd.DataFrame(face_model, columns=['Amount', 'Nose', 'Right corner of eye', 'Left corner of eye', 'Mouth', 'Right corner of mouth', 'Left corner of mouth'], index=['x', 'y', 'z'])
...
def estimate_gaze_point(accuracy_threshold, medfilt_kernel_size=7):
    ...
    try:
        ...
        print('Face_model.csv is not found or is invalid:', e)
        return
    try:
        data_frame = pd.read_csv('configs.csv', header=0)
        video_width = float(data_frame['stimuli video width (pixel)'][0])
    ...
    for file_name in os.listdir('DeepLabCutOutputs'):
        ...
        try:
            ...
            facial_landmark_coords = np.array(data_frame[['Amount', 'Amount.1', 'Nose', 'Nose.1', 'Right corner of eye', 'Right corner of eye.1', 'Left corner of eye', 'Left corner of eye.1', 'Mouth', 'Mouth.1', 'Right corner of mouth', 'Right corner of mouth.1', 'Left corner of mouth', 'Left corner of mouth.1']][1:], dtype='float32')
            facial_landmark_coords[:, 0::2] += crop_x
            facial_landmark_coords[:, 1::2] += crop_y
            is_accurate = np.float32(data_frame[['Amount.2', 'Nose.2', 'Right corner of eye.2', 'Left corner of eye.2', 'Mouth.2', 'Right corner of mouth.2', 'Left corner of mouth.2']][1:])
...
def find_aruco_marker(img, camera_parameters, aruco_marker_size, aruco_dict):
    ...
    if ids is not None:
        for i in range(len(ids)):
            ...
            if not flag:
                print('SolvePnP failed; marker index: ' + str(i) + ', ArUco_dict: ' + str(aruco_dict) + '.')
                return {}, {}
            rvec_list[int(ids[i].item())] = np.reshape(rvec, 3)
            tvec_list[int(ids[i].item())] = np.reshape(tvec, 3)
...
def find_camera(img, camera_parameters):
    ...
    for i in range(4):
        if i not in tvec_list_4x4.keys():
            print('Marker 4x4 not found, index: ' + str(i))
            return None
...
def find_monitor(img, camera_parameters):
    ...
    for i in range(4):
        if i not in tvec_list_5x5.keys():
            print('Marker 5x5 not found, index: ' + str(i))
            return None
...
def locate_setup():
    if 'camera2.pickle' not in os.listdir():
        print('Camera2.pickle is not found.')
        return
    ...
    for file_name in os.listdir('Setup'):
        ...
        if img is None:
            ...
            break
    if img is None:
        print('No image is found in Setup.')
        return
    camera_position = find_camera(img, camera2_parameters)
    monitor_position = find_monitor(img, camera2_parameters)
    if camera_position is None or monitor_position is None:
        return
    with open('setup.pickle', 'wb') as f:
        pickle.dump((camera_position, monitor_position), f)
...
def main(args):
    match args.task:
        ...
        case 'locateSetup':
            locate_setup()
        case 'estimateGazePoints':
            if args.accuracy_cutoff is None:
                print('Required inputs are incomplete.')
                return
            estimate_gaze_point(args.accuracy_cutoff)
        case 'calEstimator':
            calibrate_estimator()
        case 'calResults':
            calibrate_results()
        case 'annotateVideo':
            if args.calibrated_result_file_name is None or args.marmoset_video_name is None or args.stimuli_video_name is None or args.output_file_name is None or args.frame_delay is None or args.accuracy_cutoff is None:
                print('Required inputs are incomplete.')
                return
            annotate_video(args.calibrated_result_file_name, args.marmoset_video_name, args.stimuli_video_name, args.output_file_name, args.frame_delay, args.accuracy_cutoff)
        case _:
            raise Exception('task name should be one of the follows: createDirs calCamera1 calCamera2 locateSetup estimateGazePoints calEstimator calResults annotateVideo')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-a', '--accuracy_cutoff', type=float, help='Accuracy cutoff')
    parser.add_argument('-c', '--calibrated_result_file_name', type=str, help='Calibrated result file name')
    parser.add_argument('-d', '--frame_delay', type=int, help='Frame delay')
    parser.add_argument('-m', '--marmoset_video_name', type=str, help='Marmoset video name')
    parser.add_argument('-o', '--output_file_name', type=str, help='Output file name')
    parser.add_argument('-s', '--stimuli_video_name', type=str, help='Stimuli video name')
